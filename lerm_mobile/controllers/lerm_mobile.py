...
class MobileAppController(http.Controller):
    ...
    @http.route('/mobile_login', type='json', auth='public', methods=['POST'], csrf=False, cors='*')
    ...
    def get_app_menus(self, **kwargs):
        ...
        For kanban dashboard menus, extract action links from the kanban template.
        """
        try:
            data = request.params
            ...
            def _extract_kanban_actions(action):
                """
                If the action is a kanban view, parse its XML template and extract
                <a type="action" name="%(xml_id)d"> links as virtual child menus.
                """
                try:
                    if not hasattr(action, 'view_mode') or 'kanban' not in (action.view_mode or ''):
                        return []
                    ...
                    # Get the kanban view arch
                    Model = request.env[model_name]
                    view = Model.get_view(view_type='kanban')
                    arch = view.get('arch', '')
                    if not arch:
                        return []
                    ...
                    import re
                    doc = etree.fromstring(arch.encode('utf-8') if isinstance(arch, str) else arch)
                    
                    # Find all <a type="action" name="%(xml_id)d"> elements
                    results = []
                    virtual_id = -1
                    for a_tag in doc.iter('a'):
                        if a_tag.get('type') == 'action':
                            raw_name = a_tag.get('name', '')
                            # Extract text label
                            label = (a_tag.text or '').strip()
                            if not label:
                                # Try to get text from all children
                                label = ''.join(a_tag.itertext()).strip()
                            
                            # Also check for a heading above this <a> in sibling nodes
                            heading = ''
                            ...
                            display_name = heading if heading else label
                            if not display_name:
                                continue
                            
                            # Resolve %(xml_id)d to actual action record
                            ...
                            try:
                                action_rec = request.env.ref(xml_id)
                            except Exception:
                                continue
                            ...
                            res_model = ''
                            if hasattr(action_rec, 'res_model'):
                                res_model = action_rec.res_model or ''
                    ...
                    _logger.warning("Failed to extract kanban actions: %s", e)
                    return []
            ...
            def _get_children(parent_id):
                ...
                for menu in children:
                    ...
                    # If this is a leaf menu pointing to a kanban dashboard,
                    # extract action links as virtual children
                    if not sub_children and menu.action and action_type == 'ir.actions.act_window':
                        kanban_actions = _extract_kanban_actions(menu.action)
                        if kanban_actions:
                            sub_children = kanban_actions
    ...
    def get_records(self, **kwargs):
        ...
        try:
            ...
            # If action_id is provided from menus, try to extract its explicit domain
            action_id = data.get('action_id')
            if action_id:
                try:
                    action_rec = request.env['ir.actions.act_window'].sudo().browse(action_id)
                    if action_rec.exists() and action_rec.domain:

                        from odoo.tools.safe_eval import safe_eval
                        from odoo.tools import safe_eval as safe_eval_tools

                        safe_datetime = safe_eval_tools.datetime
                        safe_time = safe_eval_tools.time
                        ...
                        eval_context = {
                            ...
                            'time': safe_time,
                            'context_today': lambda: safe_datetime.datetime.now(),
                            'relativedelta': relativedelta,
                            'current_date': safe_datetime.date.today().strftime('%Y-%m-%d'),
                            'context': request.env.context,
                        }
                        
                        action_dom_eval = safe_eval(action_rec.domain, eval_context)
                        if isinstance(action_dom_eval, list):
                            domain.extend(action_dom_eval)
                            _logger.info("Applied Action Domain from action %s: %s", action_id, action_dom_eval)
                except Exception as e:
                    _logger.warning("Could not evaluate action domain for action_id %s: %s", action_id, e)
            ...
            _logger.info("MOBILE RECORDS — model=%s, action_id=%s, action_domain=%s, final_domain=%s",
                         model_name, action_id, action_domain, domain)
            ...
            # Build search field metadata (All non-technical fields)
            search_fields_meta = {}
            for fname, finfo in all_fields.items():
                if fname in skip_names or finfo.get('type') in skip_types:
                    continue
                if not finfo.get('string'):
                    continue
                search_fields_meta[fname] = {
                    'label': finfo.get('string', fname),
                    'type': finfo.get('type', 'char'),
                }
            ...
            return {
                ...
                'fields': fields_meta,
                'search_fields': search_fields_meta,
                'limit': limit,
                'offset': offset,
            }
    ...
    @http.route('/mobile/download_report', type='json', auth='user', methods=['POST'], csrf=False, cors='*')
    def download_report(self, **kwargs):
        """
        Call a button method on a record (e.g. print_nabl_report),
        get the ir.actions.report it returns, render the PDF,
        and return it as base64 for mobile download.
        """
        import base64
        ...
        try:
            ...
            _logger.info("DOWNLOAD REPORT payload: model=%s, id=%s, method=%s", model_name, record_id, method_name)

            if not model_name or not record_id or not method_name:
                return {'success': False, 'error': f'Missing model ({model_name}), id ({record_id}), or method ({method_name})'}
            ...
            # Call the button method (e.g. print_nabl_report)
            method = getattr(record, method_name, None)
            if not method or not callable(method):
                return {'success': False, 'error': f'Method {method_name} not found on {model_name}'}

            action = method()
            _logger.info("Report action returned: %s", action)

            if not action or action.get('type') != 'ir.actions.report':
                return {'success': False, 'error': 'Method did not return a report action'}

            report_name = action.get('report_name')
            report_data = action.get('data', {})

            if not report_name:
                return {'success': False, 'error': 'No report_name in action'}

            # Find the report action record
            ...
            # Render the PDF
            # Pass the data context (fromsample, inreport, nabl, etc.)
            # Crucially, emulate the frontend environment by putting active_id in data['context']
            report_data['context'] = {
                'active_id': record.id,
            ...
            pdf_content, content_type = report_action.with_context(
                **report_data.get('context', {})
            )._render_qweb_pdf(report_action.id, [record.id], data=report_data)

            if not pdf_content:
                return {'success': False, 'error': 'Failed to render PDF'}

            # Generate filename
            filename = f"{record.display_name or model_name}_{method_name}.pdf"
            ...
            _logger.exception("Error in download_report: model=%s, id=%s, method=%s",
                            data.get('model', ''), data.get('id', ''), data.get('method', ''))
            return {'success': False, 'error': str(e)}


    @http.route('/mobile/name_search', type='json', auth='user', methods=['POST'], csrf=False, cors='*')
    def name_search(self, **kwargs):
        """ Search for records by name in a given model. Used for Many2one field lookups. """
        data = request.params or kwargs
        try:
            ...
            results = Model.name_search(name=query, args=domain, limit=limit)
            # results is a list of (id, display_name) tuples
            return {
                'success': True,
                'records': [{'id': r[0], 'name': r[1]} for r in results],
            }

        except Exception as e:
            _logger.exception("Error in name_search for model: %s", data.get('model', ''))
            return {'success': False, 'error': str(e)}
    ...
    def read_record(self, **kwargs):
        ...
        try:
            ...
            # Resolve names for x2many fields to display richer data
    ...
    def get_model_fields(self, **kwargs):
        ...
        try:
            ...
            def _parse_node(node):
                ...
                if tag == 'header':
                ...
                elif tag in ('sheet', 'form', 'div', 'footer', 'tree', 'kanban', 'table', 'tbody', 'tr', 'td', 'strong', 'span', 'center'):
                    ...
                    if children or tag in ('tree', 'kanban'):
                        ...
                        if tag in ('tree', 'kanban'):
                            ...
                            # Instead of flattening, keep table cells intact if they have padding or structure 
                            # we might want to respect, but for simplicity we flatten containers if they just hold 1 item.
                            # Except if it's a table/tr/td, we might want to group them, but the frontend only handles 'container'.
                            # To be safe and let frontend flow handle layout, we just return the child.
                            result = children[0]
                        else:
                    ...
                    if children:
                        ...
                        # If heading has solid text and no fields, treat it as a separator
                        result = {
                            'type': 'separator',
                    ...
                    # Skip labels entirely (frontend renders its own labels for fields),
                    # but if it contains elements, we probably shouldn't just drop them.
                    # Usually, <label for="field"> doesn't contain field definitions.
                    pass
            ...
            # Extract all explicit onchange and computed methods mapped by their trigger fields
            onchange_methods_map = {}
            try:
                # 1. Add @api.onchange methods
                for fname, funcs in getattr(Model, '_onchange_methods', {}).items():
                    onchange_methods_map[fname] = [f.__name__ for f in funcs if hasattr(f, '__name__')]

                # 2. Add @api.depends methods (computed fields)
                for fname, field_obj in Model._fields.items():
                    if hasattr(field_obj, 'depends') and field_obj.depends:
                        compute_func = getattr(field_obj, 'compute', None)
                        if isinstance(compute_func, str):
                            for dep in field_obj.depends:
                                trigger = dep.split('.')[0]
                                if trigger not in onchange_methods_map:
                                    onchange_methods_map[trigger] = []
                                if compute_func not in onchange_methods_map[trigger]:
                                    onchange_methods_map[trigger].append(compute_func)
            except Exception as e:
                _logger.warning("Could not determine onchange methods for %s: %s", model, e)
            ...
            for fname in seen_fields:
                if fname in all_fields:
                    ...
                    clean_fields[fname] = {
                        ...
                        'has_onchange': len(onchange_methods_map.get(fname, [])) > 0,
                        'onchange_methods': onchange_methods_map.get(fname, []),
                    }
    ...
    @http.route('/mobile/onchange', type='json', auth='user', methods=['POST'], csrf=False, cors='*')
    def trigger_onchange(self, **kwargs):
        """
        Trigger Odoo's onchange mechanism for a model.
        This handles both @api.onchange and @api.depends (computed field) recomputation.

        Expected params:
            model: str - The model name (e.g., 'lerm.srf.sample')
            record_id: int - The record ID (0 for new records)
            values: dict - Current form field values
            field_name: str - The field that just changed (or list of fields)
            field_names: list - All field names present in the form view (for building onchange_spec)
        
        Returns:
            success: bool
            changes: dict - Field values that changed as a result of onchange
            domain: dict - Updated domains for relational fields (optional)
            warning: dict - Warning message to display (optional)
        """
        # import wdb;wdb.set_trace()
        data = request.params or kwargs
        try:
            ...
            Model = request.env[model]
            all_fields = Model.fields_get()
            methods_to_call = data.get('methods_to_call', [])
            method_to_call = data.get('method_to_call')
            
            if method_to_call and not methods_to_call:
                methods_to_call = [method_to_call]
            ...
            for key, val in raw_values.items():
                if key not in all_fields:
                    continue
                field_type = all_fields[key].get('type')
                    ...
                    if isinstance(val, list):
                        ...
                        for item in val:
                            if isinstance(item, int):
                                ...
                                ids.append(item[0])
                        clean_values[key] = [(6, 0, ids)]
                    else:
                        clean_values[key] = val if val else [(6, 0, [])]
                elif val == '' and field_type in ('integer', 'float', 'monetary', 'date', 'datetime'):
                    clean_values[key] = False
                elif val is None:
                    clean_values[key] = False
                else:
            ...
            # Keep ID if present for initial record binding
            if record_id:
                origin_record = Model.browse(record_id)
                record = Model.new(clean_values, origin=origin_record)
            else:
                record = Model.new(clean_values)
                ...
                for method_name in methods_to_call:
                    if hasattr(record, method_name):
                        try:
                            ...
                            _logger.warning("Failed calling %s: %s", method_name, e)

            # Compute the changes by comparing with what was passed in
            changes = {}
            domain = {}
            warning = None
            ...
            for fname in all_fields:
                try:
                    fval = record[fname]
                    # Format recordset value to comparable types
                    field_type = all_fields[fname].get('type')
                    if field_type == 'many2one':
                        if fval and fval.id:
                            # if it changed from what was initially passed (which is an int)
                            if fval.id != clean_values.get(fname):
                                changes[fname] = [fval.id, fval.display_name]
                        elif not fval and clean_values.get(fname):
                             changes[fname] = False
                    elif field_type in ('one2many', 'many2many'):
                        new_ids = fval.ids
                        # Simple comparison for ids arrays
                        # clean_values has [(6,0,ids)] format
                        old_val = clean_values.get(fname)
                        old_ids = []
                        if isinstance(old_val, list) and len(old_val) > 0:
                            old_ids = old_val[0][2] if isinstance(old_val[0], (list, tuple)) and len(old_val[0]) == 3 else []
                        if set(new_ids) != set(old_ids):
                            changes[fname] = new_ids
                    else:
                        if fval != clean_values.get(fname):
                             # Dates etc need formatting but keeping simple
                             changes[fname] = fval
                except Exception:
                    continue
            ...
            _logger.exception("Error triggering onchange for model %s, field %s", data.get('model', ''), data.get('field_name', ''))
            return {'success': False, 'error': str(e)}


    @http.route('/mobile/render_pdf', type='json', auth='user', methods=['POST'], csrf=False, cors='*')
    def mobile_render_pdf(self, **kwargs):
        """
        Generates a PDF report and returns it as base64 for the mobile app.
        Uses JSON-RPC POST (same auth as all other mobile endpoints).
        """
        try:
            import base64
            data = request.params or kwargs
            ...
            if not report_name or not record_id:
                return {'success': False, 'error': 'report_name and id are required'}
            ...
            if isinstance(report_data, dict) and 'context' not in report_data:
                report_data['context'] = context

            report_sudo = request.env['ir.actions.report'].sudo()._get_report_from_name(report_name)
            if not report_sudo:
                _logger.warning("Mobile PDF: _get_report_from_name returned nothing for %s, trying search", report_name)
                report_sudo = request.env['ir.actions.report'].sudo().search([('report_name', '=', report_name)], limit=1)
            if not report_sudo:
                return {'success': False, 'error': f'Report not found: {report_name}'}

            # Render the PDF server-side
