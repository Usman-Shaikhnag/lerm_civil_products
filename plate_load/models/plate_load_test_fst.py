...
class PlateLoadTest(models.Model):
    ...
    def _compute_eln_data(self):
        for record in self:
            if record.eln_ref:
                ...
                record.report_no = eln.eln_id
                record.ulr_no = sample.ulr_no if sample else ""
                
                # Fetching test method from the first parameter
                record.references = eln.parameters[0].test_method.test_method if eln.parameters else ""
    ...
    def open_form(self):
        ...
        react_url = f"http://147.93.154.53:5173/temp_monitoring/plate_load_test?token={token}"
