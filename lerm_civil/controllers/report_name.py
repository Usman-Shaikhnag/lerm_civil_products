...
class ReportDownloadControllerSteelChemical(http.Controller):
    @http.route(['/download_report/steel/nabl/<int:eln_id>'], type='http', auth='public', website=True, csrf=False)
    def download_report_nabl_steel(self, eln_id, **kw):
        try:
            eln = request.env['lerm.eln'].sudo().browse(eln_id)
            if not eln.exists():
                return werkzeug.exceptions.NotFound("ELN record not found")

            report_name = 'steel_chemical.steel_chemical_report'
            pdf_content, _ = request.env['ir.actions.report']._render_qweb_pdf(
                report_name, res_ids=[eln.id], data={'nabl': True}
            )
    ...
    @http.route(['/download_report/steel/nonnabl/<int:eln_id>'], type='http', auth='public', website=True, csrf=False)
    def download_report_nonnabl_steel(self, eln_id, **kw):
        try:
            eln = request.env['lerm.eln'].sudo().browse(eln_id)
            if not eln.exists():
                return werkzeug.exceptions.NotFound("ELN record not found")
