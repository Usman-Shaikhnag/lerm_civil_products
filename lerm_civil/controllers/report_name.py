...
class ReportDownloadControllerCoarse(http.Controller):
    ...
    def download_report_nabl_coarse(self, eln_id, **kw):
        try:
            ...
            report_name = 'coarse_aggregate2.lerm_coarse_aggregate_mech_report2'
            pdf_content, _ = request.env['ir.actions.report']._render_qweb_pdf(
                report_name, res_ids=[eln.id], data={'nabl': True}
            )
...
class ReportDownloadControllerSolidConcrete(http.Controller):
    @http.route(['/download_report/solid_concrete/nabl/<int:eln_id>'], type='http', auth='public', website=True, csrf=False)
    def download_report_nabl_solid_concrete(self, eln_id, **kw):
        try:
            eln = request.env['lerm.eln'].sudo().browse(eln_id)
            if not eln.exists():
                return werkzeug.exceptions.NotFound("ELN record not found")

            report_name = 'solid_concrete_block.solid_concrete_report'
            pdf_content, _ = request.env['ir.actions.report']._render_qweb_pdf(
                report_name, res_ids=[eln.id], data={'nabl': True}
            )
    ...
    @http.route(['/download_report/solid_concrete/nonnabl/<int:eln_id>'], type='http', auth='public', website=True, csrf=False)
    def download_report_nonnabl_solid_concrete(self, eln_id, **kw):
        try:
            eln = request.env['lerm.eln'].sudo().browse(eln_id)
            if not eln.exists():
                return werkzeug.exceptions.NotFound("ELN record not found")
...
class ReportDownloadControllerGypsumPlaster(http.Controller):
    @http.route(['/download_report/gypsump/nabl/<int:eln_id>'], type='http', auth='public', website=True, csrf=False)
    def download_report_nabl_gypsump(self, eln_id, **kw):
        try:
            eln = request.env['lerm.eln'].sudo().browse(eln_id)
            if not eln.exists():
                return werkzeug.exceptions.NotFound("ELN record not found")

            report_name = 'gypsum_plaster_board.gypsum_plaster_board_report'
            pdf_content, _ = request.env['ir.actions.report']._render_qweb_pdf(
                report_name, res_ids=[eln.id], data={'nabl': True}
            )
    ...
    @http.route(['/download_report/gypsump/nonnabl/<int:eln_id>'], type='http', auth='public', website=True, csrf=False)
    def download_report_nonnabl_gypsump(self, eln_id, **kw):
        try:
            eln = request.env['lerm.eln'].sudo().browse(eln_id)
            if not eln.exists():
                return werkzeug.exceptions.NotFound("ELN record not found")
...
class ReportDownloadControllerParticleBoard(http.Controller):
    @http.route(['/download_report/particleb/nabl/<int:eln_id>'], type='http', auth='public', website=True, csrf=False)
    def download_report_nabl_particleb(self, eln_id, **kw):
        try:
            eln = request.env['lerm.eln'].sudo().browse(eln_id)
            if not eln.exists():
                return werkzeug.exceptions.NotFound("ELN record not found")

            report_name = 'particle_board.particle_board_report'
            pdf_content, _ = request.env['ir.actions.report']._render_qweb_pdf(
                report_name, res_ids=[eln.id], data={'nabl': True}
            )
    ...
    @http.route(['/download_report/particleb/nonnabl/<int:eln_id>'], type='http', auth='public', website=True, csrf=False)
    def download_report_nonnabl_particleb(self, eln_id, **kw):
        try:
            eln = request.env['lerm.eln'].sudo().browse(eln_id)
            if not eln.exists():
                return werkzeug.exceptions.NotFound("ELN record not found")
...
class ReportDownloadControllerCubeDensity(http.Controller):
    @http.route(['/download_report/cubedensity/nabl/<int:eln_id>'], type='http', auth='public', website=True, csrf=False)
    def download_report_nabl_cubedensity(self, eln_id, **kw):
        try:
            eln = request.env['lerm.eln'].sudo().browse(eln_id)
            if not eln.exists():
                return werkzeug.exceptions.NotFound("ELN record not found")

            report_name = 'concrete_cube_density.concrete_cube_density_report'
            pdf_content, _ = request.env['ir.actions.report']._render_qweb_pdf(
                report_name, res_ids=[eln.id], data={'nabl': True}
            )
    ...
    @http.route(['/download_report/cubedensity/nonnabl/<int:eln_id>'], type='http', auth='public', website=True, csrf=False)
    def download_report_nonnabl_cubedensity(self, eln_id, **kw):
        try:
            eln = request.env['lerm.eln'].sudo().browse(eln_id)
            if not eln.exists():
                return werkzeug.exceptions.NotFound("ELN record not found")
...
class ReportDownloadControllerCoreDensity(http.Controller):
    @http.route(['/download_report/coredensity/nabl/<int:eln_id>'], type='http', auth='public', website=True, csrf=False)
    def download_report_nabl_coredensity(self, eln_id, **kw):
        try:
            eln = request.env['lerm.eln'].sudo().browse(eln_id)
            if not eln.exists():
                return werkzeug.exceptions.NotFound("ELN record not found")

            report_name = 'concrete_core_density.concrete_core_density_report'
            pdf_content, _ = request.env['ir.actions.report']._render_qweb_pdf(
                report_name, res_ids=[eln.id], data={'nabl': True}
            )
    ...
    @http.route(['/download_report/coredensity/nonnabl/<int:eln_id>'], type='http', auth='public', website=True, csrf=False)
    def download_report_nonnabl_coredensity(self, eln_id, **kw):
        try:
            eln = request.env['lerm.eln'].sudo().browse(eln_id)
            if not eln.exists():
                return werkzeug.exceptions.NotFound("ELN record not found")
...
class ReportDownloadControllerNaturalStone(http.Controller):
    @http.route(['/download_report/naturalstone/nabl/<int:eln_id>'], type='http', auth='public', website=True, csrf=False)
    def download_report_nabl_naturalstone(self, eln_id, **kw):
        try:
            eln = request.env['lerm.eln'].sudo().browse(eln_id)
            if not eln.exists():
                return werkzeug.exceptions.NotFound("ELN record not found")

            report_name = 'natural_bulding_stone.natural_building_stone_report'
            pdf_content, _ = request.env['ir.actions.report']._render_qweb_pdf(
                report_name, res_ids=[eln.id], data={'nabl': True}
            )
    ...
    @http.route(['/download_report/naturalstone/nonnabl/<int:eln_id>'], type='http', auth='public', website=True, csrf=False)
    def download_report_nonnabl_naturalstone(self, eln_id, **kw):
        try:
            eln = request.env['lerm.eln'].sudo().browse(eln_id)
            if not eln.exists():
                return werkzeug.exceptions.NotFound("ELN record not found")
...
class ReportDownloadControllerCubeWaterAbsorption(http.Controller):
    @http.route(['/download_report/waterab/nabl/<int:eln_id>'], type='http', auth='public', website=True, csrf=False)
    def download_report_nabl_waterab(self, eln_id, **kw):
        try:
            eln = request.env['lerm.eln'].sudo().browse(eln_id)
            if not eln.exists():
                return werkzeug.exceptions.NotFound("ELN record not found")

            report_name = 'concrete_cube_water_absorption.cube_absorption_report'
            pdf_content, _ = request.env['ir.actions.report']._render_qweb_pdf(
                report_name, res_ids=[eln.id], data={'nabl': True}
            )
    ...
    @http.route(['/download_report/waterab/nonnabl/<int:eln_id>'], type='http', auth='public', website=True, csrf=False)
    def download_report_nonnabl_waterab(self, eln_id, **kw):
        try:
            eln = request.env['lerm.eln'].sudo().browse(eln_id)
            if not eln.exists():
                return werkzeug.exceptions.NotFound("ELN record not found")
...
class ReportDownloadControllerSSTMTBAR(http.Controller):
    @http.route(['/download_report/sstmtbar/nabl/<int:eln_id>'], type='http', auth='public', website=True, csrf=False)
    def download_report_nabl_sstmtbar(self, eln_id, **kw):
        try:
            eln = request.env['lerm.eln'].sudo().browse(eln_id)
            if not eln.exists():
                return werkzeug.exceptions.NotFound("ELN record not found")

            report_name = 'ss_tmt_bar.stainless_steel_tmt_bar_report'
            pdf_content, _ = request.env['ir.actions.report']._render_qweb_pdf(
                report_name, res_ids=[eln.id], data={'nabl': True}
            )
    ...
    @http.route(['/download_report/sstmtbar/nonnabl/<int:eln_id>'], type='http', auth='public', website=True, csrf=False)
    def download_report_nonnabl_sstmtbar(self, eln_id, **kw):
        try:
            eln = request.env['lerm.eln'].sudo().browse(eln_id)
            if not eln.exists():
                return werkzeug.exceptions.NotFound("ELN record not found")
