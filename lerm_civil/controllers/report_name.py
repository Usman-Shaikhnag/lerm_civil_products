...
class ReportDownloadController(http.Controller):
    ...
    def download_report_nabl(self, eln_id, **kw):
        ...
        if eln.is_product_based_calculation and eln.material.product_based_calculation:
            rpt = eln.material.product_based_calculation[0].main_report_template
            template_name = rpt.report_name if isinstance(rpt.report_name, str) else rpt.report_name.id if rpt.report_name else False
        elif eln.parameters_result.parameter:
            rpt = eln.parameters_result.parameter[0].main_report_template
            template_name = rpt.report_name if isinstance(rpt.report_name, str) else rpt.report_name.id if rpt.report_name else False

        if not template_name:
            return request.not_found()

        # ensure string report name
        if not isinstance(template_name, str):
            template_name = request.env['ir.actions.report'].sudo().browse(template_name).report_name
