...
# class LermReportPortal(CustomerPortal):
#     def _lerm_report_portal_values(self, counters):
#         rtn = super(LermReportPortal, self)._lerm_report_portal_values(counters)
#         rtn['kes_no'] = request.env['lerm.srf.sample'].search_count([])
#         print("_lerm_report_portal_values",rtn)
#         return rtn
...
#         return request.render("lerm_civil.portal_my_report",vals)


#     @http.route(['/my/sample/<model("lerm.srf.sample"):kes_no>'], type='http', website=True)
#     def WebLermSampleView(self, kes_no, **kw):
#         vals = {"kes": kes_no, 'page_name': 'sample_from_view'}
#         print('<<<<<<<<<<<<<<<<<<<<<<<',vals)  # Correct the variable name to kes_no
#         return request.render("lerm_civil.wb_sample_from_view_portal", vals)
