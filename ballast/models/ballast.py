...
class BallastMechanical(models.Model):
    ...
    average_impact_value_conformity = fields.Selection([
        ...
        ('fail', 'Fail'),
        ('na', 'NA'),
    ], string='Conformity',compute="_compute_average_impact_value_conformity")
    ...
    avg_water_absorption_conformity = fields.Selection([
        ...
        ('fail', 'Fail'),
        ('na', 'NA'),
    ], string='Conformity',compute="_compute_avg_water_absorption_conformity")
    ...
    avg_abrasion_value_conformity = fields.Selection([
        ...
        ('fail', 'Fail'),
        ('na', 'NA'),
    ], string='Conformity',compute="_compute_avg_abrasion_value_conformity")
    ...
    bulk_density_line_ids = fields.One2many('ballast.bulk.density.line', 'parent_id', string="Observations")

    avg_bulk_density = fields.Float(
        "Average Bulk Density  (Kg/Lit)",
    ...
    def _compute_avg_bulk_density(self):
        for rec in self:
            lines = rec.bulk_density_line_ids

            if lines:
                values = lines.mapped('bulk_density')
                rec.avg_abrasion_value = sum(values) / len(values)
            else:
                rec.avg_abrasion_value = 0.0 
    ...
    avg_bulk_density_conformity = fields.Selection([
        ...
        ('fail', 'Fail'),
        ('na', 'NA'),
    ], string='Conformity',compute="_compute_avg_bulk_density_conformity")

    avg_bulk_density_nabl = fields.Selection([
        ('pass', 'NABL'),
        ('fail', 'Non-NABL'),
    ], string='NABL', default='fail',compute="_compute_avg_bulk_density_nabl")
    ...
    def _compute_avg_bulk_density_conformity(self):
        for record in self:
            ...
            line = self.env['lerm.parameter.master'].sudo().search([('internal_id','=','862548324-86e4-424a-816f-cd698745621l')])
            materials = self.env['lerm.parameter.master'].sudo().search([('internal_id','=','862548324-86e4-424a-816f-cd698745621l')]).parameter_table
            mu_value = line.mu_value
            for material in materials:
                if material.grade.id == record.grade.id:
                    req_min = material.req_min
    ...
    def _compute_visible(self):
        for record in self:
            ...
            for sample in record.sample_parameters:
                ...
                if sample.internal_id == '862548324-86e4-424a-816f-cd698745621l':
    ...
    def open_eln_page(self):
        ...
        for result in technician_results:
            ...
            if result.parameter.internal_id == '862548324-86e4-424a-816f-cd698745621l':
                result.calculated = True
...
class BallastBulkDensityLine(models.Model):
    _name = "ballast.bulk.density.line"
    parent_id = fields.Many2one('mechanical.ballast',string="Parent Id")

    sample_no = fields.Integer(string="Trial No", readonly=True, copy=False, default=1)

    w = fields.Float("Weight of Material in Container after pouring, W  (Kg) ")
    v = fields.Float("Volume of calibrating container ,V (Lit)")

    

    bulk_density = fields.Float(
        string="Bulk Density of Material, W/V (Kg/Lit)",
        compute="_compute_bulk_density",
        store=True,
    )
