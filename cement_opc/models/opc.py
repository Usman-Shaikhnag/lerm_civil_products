...
class CementNormalConsistency(models.Model):
    ...
    lab_opc_ids = fields.One2many(
     ...
     # --- Button Function ---
    def action_generate_options_opc(self):
        for record in self:
            ...
            if record.lab_id and '-' in record.lab_id:
                try:
                    ...
                    if len(parts) >= 2:
                        ...
                        record.lab_opc_ids = lines_command
                        
                except Exception as e:
                    pass
            else:
                if record.lab_id:
                    record.lab_opc_ids = [(5, 0, 0), (0, 0, {'lab': record.lab_id})]
    ...
    # selected_consistency_cement = fields.Many2one(
    #     'opc.lab.line',
    #     string="Select Lab ID",
    #     domain="[('id', 'in', lab_opc_ids)]"
    # )
    ...
    def action_generate_opc_consistency_cement(self):
        for record in self:
    ...
    def action_generate_opc_intial_time(self):
        for record in self:
    ...
    compressive_generated = fields.Boolean(string="Compressive Lab Lines ",default=False)
    show_sieve = fields.Boolean(default=False)

    def action_generate_opc_compressive_lines(self):
        for record in self:
            if record.lab_id and ' - ' in record.lab_id:
                start_str, end_str = record.lab_id.split(' - ')
                prefix = '-'.join(start_str.split('-')[:2])
                ...
                record.opc_compressive_ids = lines
                record.compressive_generated = True

            # 🔹 Set flag to show sieve analysis
            if record.opc_compressive_ids:
                record.show_sieve = True
    ...
    def action_generate_ggbs_soundness_le_method_lines(self):
        for record in self:
            if record.lab_id and ' - ' in record.lab_id:
                start_str, end_str = record.lab_id.split(' - ')
                prefix = '-'.join(start_str.split('-')[:2])
                ...
                record.opc_le_method_ids = lines
                record.soundness_le_method_generated = True

            # 🔹 Set flag to show sieve analysis
            if record.opc_le_method_ids:
                record.show_sieve = True
...
class LabOptionLine(models.Model):
    ...
    parent_id = fields.Many2one('cement.opc', string="Parent")
