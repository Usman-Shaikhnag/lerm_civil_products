...
class CementNormalConsistency(models.Model):
    ...
    name = fields.Char("Name",default="Cement OPC")
    ...
    intial_time_lines = fields.One2many('cement.setting.time.initial.line','parent_id',string="Initial Time")

    final_time_lines = fields.One2many('cement.setting.time.final.line','parent_id',string="Initial Time")
    ...
    compressive_lines = fields.One2many('compressive.line','parent_id',string="Compressive",default=lambda self: self.compressive_lines_days())
    ...
    def _compute_avg_28_days_confirmity(self):
        for record in self:
            ...
            line = self.env['lerm.parameter.master'].sudo().search([('internal_id','=','b48d3c45-a2d6-4b69-a277-4f771724efdb')])
            materials = self.env['lerm.parameter.master'].sudo().search([('internal_id','=','b48d3c45-a2d6-4b69-a277-4f771724efdb')]).parameter_table
            for material in materials:
                if material.grade.id == record.grade.id:
                    req_min = material.req_min
                    req_max = material.req_max
    ...
    fineness_blaine_ids = fields.One2many("fineness.blaine.line","parent_id",string="Trial Lines")
    ...
    def open_eln_page(self):
        ...
        for result in technician_results:
            ...
            if result.parameter.internal_id == 'b48d3c45-a2d6-4b69-a277-4f771724efdb':
            ...
            # Drying Skrinkage
            if result.parameter.internal_id == '20246345-407d-4ce8-ae0d-566bd4e3b52f':
...
class CementSettingTimeInitialLine(models.Model):
    _name = 'cement.setting.time.initial.line'
...
class CementSettingTimeFinalLine(models.Model):
    _name = 'cement.setting.time.final.line'
...
class CementSpecificGravityLine(models.Model):
    _name = "cement.specific.gravity.line"
    ...
    def create(self, vals):
        ...
        return super(CementSpecificGravityLine, self).create(vals)
...
class FinenessBlaineLine(models.Model):
    ...
    parent_id = fields.Many2one('cement.opc',string="Parent Id",ondelete="cascade",)
