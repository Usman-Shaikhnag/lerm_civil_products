...
class UpvCombined(models.Model):
    _name = "ndt.upv.combined"
    _inherit = "lerm.eln"
    _rec_name = "name"
    ...
    child_lines1 = fields.One2many('ndt.upv.combined.line1', 'parent_id', string="Parameter")
    ...
    @api.depends('child_lines1.velocity')
    def _compute_velocity_stats(self):
        for record in self:
            velocities = record.child_lines1.mapped('velocity')
            if velocities:
    ...
    child_lines2 = fields.One2many('ndt.upv.combined.line2', 'parent_id', string="Parameter")
    ...
    notes_id = fields.One2many('ndt.upv.combined.notes', 'parent_id', string="Notes")
    
    @api.model
    def default_get(self, fields):
        res = super(UpvCombined, self).default_get(fields)
    ...
    def open_eln_page(self):
        ...
        for result in technician_results:
            if result.parameter.internal_id == '1e7a5df3-a897-4933-bd5a-2798ijhyut56':
    ...
    def create(self, vals):
        # import wdb;wdb.set_trace()
        record = super(UpvCombined, self).create(vals)
        record.parameter_id.write({'model_id':record.id})
        return record


class UpvCombinedLine1(models.Model):
    _name = "ndt.upv.combined.line1"
    parent_id = fields.Many2one('ndt.upv.combined',string="Parent Id")
    ...
    def create(self, vals):
        ...
        return super(UpvCombinedLine1, self).create(vals)
...
class UpvCombinedLine2(models.Model):
    _name = "ndt.upv.combined.line2"
    parent_id = fields.Many2one('ndt.upv.combined',string="Parent Id")
    ...
    def _compute_dt_of_casting(self):
        for record in self:
            ...
            if not record.parent_id or not record.parent_id.child_lines2:
                record.casting_date = False
                continue

            all_lines = list(record.parent_id.child_lines2)
    ...
    @api.depends('parent_id.child_lines2', 'parent_id.child_lines2.dist', 'parent_id.child_lines2.time')
    ...
    def create(self, vals):
        ...
        return super(UpvCombinedLine2, self).create(vals)

    def _reorder_serial_numbers(self):
        # Reorder the serial numbers based on the positions of the records in child_lines2
        records = self.sorted('id')
        for index, record in enumerate(records):
            record.serial_no = index + 1

class UpvNotes(models.Model):
    _name = "ndt.upv.combined.notes"
