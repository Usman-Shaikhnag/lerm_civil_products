...
class CementPPCPrefillWizard(models.TransientModel):
    ...
    def prefill_data(self):
        # Current active Cement PPC record
        current_product = self.env['cement.ppc'].sudo().browse(self._context['active_id'])

        # Fetch previous Cement PPC record for selected sample
        copy_product = self.env['cement.ppc'].sudo().search([
            ('eln_ref.sample_id', '=', self.sample_id.id)   # <<< FIXED
        ], limit=1)

        if not copy_product:
            raise UserError("Selected sample does not have any previous Cement PPC record.")
