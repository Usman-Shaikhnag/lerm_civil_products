...
class CementPPC(models.Model):
    ...
    intial_time_lines = fields.One2many('cement.ppc.setting.time.initial.line','parent_id',string="Initial Time")

    final_time_lines = fields.One2many('cement.ppc.setting.time.final.line','parent_id',string="Initial Time")
    ...
    def _compute_final_setting_time_confirmity(self):
        for record in self:
            ...
            line = self.env['lerm.parameter.master'].sudo().search([('internal_id','=','3214ght-5e9c-4335-9ea2-2d87624c3061')])
            materials = self.env['lerm.parameter.master'].sudo().search([('internal_id','=','3214ght-5e9c-4335-9ea2-2d87624c3061')]).parameter_table
            for material in materials:
                if material.grade.id == record.grade.id:
                    req_min = material.req_min
                    req_max = material.req_max
    ...
    compressive_lines = fields.One2many('compressive.ppc.line','parent_id',string="Compressive",default=lambda self: self.compressive_lines_days())
    ...
    def _compute_avg_14_days_confirmity(self):
        for record in self:
            ...
            line = self.env['lerm.parameter.master'].sudo().search([('internal_id','=','b1704bab-6d5a-4f39-8092-a0aeba36fdfd')])
            materials = self.env['lerm.parameter.master'].sudo().search([('internal_id','=','b1704bab-6d5a-4f39-8092-a0aeba36fdfd')]).parameter_table
            for material in materials:
                if material.grade.id == record.grade.id:
                    req_min = material.req_min
                    req_max = material.req_max
    ...
    specific_gravity_line_ids = fields.One2many(
        "cement.ppc.specific.gravity.line",
        "parent_id",
        string="Trial Lines",
    )
    ...
    def open_eln_page(self):
        ...
        for result in technician_results:
            ...
            if result.parameter.internal_id == 'b1704bab-6d5a-4f39-8092-a0aeba36fdfd':
    ...
    notes_id = fields.One2many('cement.ppc.notes', 'parent_id', string="Notes", default=lambda self: self._default_notes_lines())
...
class FinenessCementPPCLine(models.Model):
    ...
    def create(self, vals):
        ...
        return super(FinenessCementPPCLine, self).create(vals)
...
class ConsistencyCementPPCLine(models.Model):
    ...
    def create(self, vals):
        ...
        return super(ConsistencyCementPPCLine, self).create(vals)
...
class CementPPCSettingTimeInitialLine(models.Model):
...
class CemenPPCSettingTimeFinalLine(models.Model):
...
class CompressiveCementPPCLine(models.Model):
    ...
    def create(self, vals):
        ...
        return super(CompressiveCementPPCLine, self).create(vals)
...
class soundnessCementPPCLine(models.Model):
    ...
    def create(self, vals):
        ...
        return super(soundnessCementPPCLine, self).create(vals)
...
class CementSpecificGravityPPCLine(models.Model):
    ...
    def create(self, vals):
        ...
        return super(CementSpecificGravityPPCLine, self).create(vals)
...
class DensityCementPPCLine(models.Model):
    ...
    def create(self, vals):
        ...
        return super(DensityCementPPCLine, self).create(vals)
...
class FinenessBlainePPCLine(models.Model):
    ...
    def create(self, vals):
        ...
        return super(FinenessBlainePPCLine, self).create(vals)
