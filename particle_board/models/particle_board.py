...
class ParticleBoard(models.Model):
    ...
    notes_id = fields.One2many('mechanical.particle.board.notes', 'parent_id', string="Notes")
    
    @api.model
    def default_get(self, fields):
        res = super(ParticleBoard, self).default_get(fields)
    ...
    average_density_particle_conformity = fields.Selection([
            ...
            ('fail', 'Fail'),
            ('--', '--')
            ], string="Density Conformity", compute="_compute_average_density_particle_conformity", store=True)
        ...
        for record in self:
            ...
            for material in materials:
                if material.grade.id == record.grade.id:
                    ...
                    # Check if permissible limit is '--' or empty
                    if hasattr(material, 'permissable_limit') and (material.permissable_limit == '--' or not material.permissable_limit):
                        record.average_density_particle_conformity = '--'
                        break
    ...
    average_moisture_particle_conformity = fields.Selection([
            ...
            ('fail', 'Fail'),
            ('--', '--')
            ], string="Moisture Content Conformity", compute="_compute_average_moisture_particle_conformity", store=True)
        ...
        for record in self:
            ...
            for material in materials:
                if material.grade.id == record.grade.id:
                    ...
                    # Check if permissible limit is '--' or empty
                    if hasattr(material, 'permissable_limit') and (material.permissable_limit == '--' or not material.permissable_limit):
                        record.average_moisture_particle_conformity = '--'
                        break
    ...
    average_water_absorption_particle_conformity = fields.Selection([
            ...
            ('fail', 'Fail'),
            ('--', '--')
            ], string="Conformity", compute="_compute_average_water_absorption_particle_conformity", store=True)
        ...
        for record in self:
            ...
            for material in materials:
                if material.grade.id == record.grade.id:
                    ...
                    # Check if permissible limit is '--' or empty
                    if hasattr(material, 'permissable_limit') and (material.permissable_limit == '--' or not material.permissable_limit):
                        record.average_water_absorption_particle_conformity = '--'
                        break
    ...
    def open_eln_page(self):
        ...
        for result in technician_results:
            ...
            if result.parameter.internal_id == '261b79c2-47c9-4662-9298-3095ac900ffc':
            ...
            if result.parameter.internal_id == '5685faa4-b637-4eb2-8d5e-7c0483f3938c':
                # result.result_char = self.initial_setting_time_minutes_unrounded
                result.calculated = True
                # if self.initial_setting_nabl == 'pass':
                #     result.nabl_status = 'nabl'
            ...
            if result.parameter.internal_id == '4be06eae-a5c2-49e0-bf58-40a077b6f408':
                result.result_char = round(self.average_water_absorption_particle,2)
                result.calculated = True
                if self.average_water_absorption_particle_nabl == 'pass':
                    result.nabl_status = 'nabl'
                else:
                    result.nabl_status = 'non-nabl'
                continue
            ...
            if result.parameter.internal_id == '7ab10ef0-5dff-4e6c-97a2-2e2e51a83e02':
            ...
            if result.parameter.internal_id == '23ab64c3-0ae9-4369-bac5-983ba20d514f':
            ...
            if result.parameter.internal_id == 'eb40f6c4-c96a-46e2-ab3d-0ee0a30a2f3a':
            ...
            if result.parameter.internal_id == '22079c87-a064-4501-a638-e9fff7b109f6':
            ...
            if result.parameter.internal_id == 'c2db5d8c-f3e8-49d4-a56f-577e81e1d0e8':
            ...
            if result.parameter.internal_id == 'f6e482b7-6b54-40ea-b96e-313f38794876':
                result.result_char = round(self.average_density_particle,2)
                result.calculated = True
                if self.average_density_particle_nabl == 'pass':
                    result.nabl_status = 'nabl'
                else:
                    result.nabl_status = 'non-nabl'
                continue

            if result.parameter.internal_id == '9f5db24b-40f4-49bd-a490-3a001cded1c9':
                result.result_char = round(self.average_moisture_particle,2)
                result.calculated = True
                if self.average_moisture_particle_nabl == 'pass':
                    result.nabl_status = 'nabl'
                else:
                    result.nabl_status = 'non-nabl'
                continue
...
class ShutteringPlywoodNotes(models.Model):
    _name = "mechanical.particle.board.notes"
