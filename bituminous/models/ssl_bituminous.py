...
class BituminousMechanical(models.Model):
    ...
    combined_gradation_child_lines = fields.One2many('bituminous.combined.gradation.line','parent_id',string="Parameter")
        ...
        for record in self:
            for line in record.combined_gradation_child_lines:
                ...
                if previous_line == 0:
                    ...
                    previous_line_record = self.env['bituminous.combined.gradation.line'].sudo().search([("serial_no", "=", previous_line),("parent_id","=",self.id)]).cumulative_retained
...
class CombinedGradationLine(models.Model):
    _name = "bituminous.combined.gradation.line"
    parent_id = fields.Many2one('bituminous', string="Parent Id")
