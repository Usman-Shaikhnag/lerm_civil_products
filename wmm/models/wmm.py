...
class WmmMechanical(models.Model):
    _name = "mechanical.wmm1"
    _inherit = "lerm.eln"
    _description = 'mechanical.wmm1'
    _rec_name = "name"
    ...
    def get_all_fields(self):
        record = self.env['mechanical.wmm1'].browse(self.ids[0])
        field_values = {}
    ...
    dry_gradation_table = fields.One2many('mech.dry.gradation.line1','parent_id',string="Dry Gradation")
        ...
        for record in self:
            for line in record.dry_gradation_table:
                ...
                if previous_line == 0:
                    ...
                    previous_line_record = self.env['mech.dry.gradation.line1'].sudo().search([("serial_no", "=", previous_line),("parent_id","=",self.id)]).cumulative_retained
    ...
    elongation_table = fields.One2many('mech.elongation.flakiness.line1','parent_id',string="Elongation Flakiness Index")
    ...
    impact_value_child_lines = fields.One2many('mech.wmm.impact.line1','parent_id',string="Parameter")
    ...
    liquid_limit_table = fields.One2many('mech.wmm.liquid.limit.line1','parent_id',string="Liquid Limit")
    ...
    plastic_table = fields.One2many('mech.wmm.plastic.limit.line1','parent_id',string="Plastic Limit")
    ...
    density_relation_table = fields.One2many('mech.wmm.density.relation.line1','parent_id',string="Density Relation")
    ...
    cbr_table = fields.One2many('mechanical.wmm.cbr.line1','parent_id',string="CBR")
...
class WmmDensityRelationLine(models.Model):
    _name = "mech.wmm.density.relation.line1"
    parent_id = fields.Many2one('mechanical.wmm1',string="Parent Id")
...
class WmmCBRLine(models.Model):
    _name = "mechanical.wmm.cbr.line1"
    parent_id = fields.Many2one('mechanical.wmm1',string="Parent Id")
...
class WmmLiquidLimitLine(models.Model):
    _name = "mech.wmm.liquid.limit.line1"
    parent_id = fields.Many2one('mechanical.wmm1', string="Parent Id")
...
class WmmPlasticLimitLine(models.Model):
    _name = "mech.wmm.plastic.limit.line1"
    parent_id = fields.Many2one('mechanical.wmm1', string="Parent Id")
...
class DryGradationLine(models.Model):
    _name = "mech.dry.gradation.line1"
    parent_id = fields.Many2one('mechanical.wmm1', string="Parent Id")
...
class ElongationLine(models.Model):
    _name = "mech.elongation.flakiness.line1"
    parent_id = fields.Many2one('mechanical.wmm1', string="Parent Id")
