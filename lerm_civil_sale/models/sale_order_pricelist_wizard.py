...
class SaleOrderPricelistWizard(models.TransientModel):
    ...
    def _build_line_data(self, order):
        ...
        per parameter; lines without parameters produce a single whole-product
        row (parameter_id = False). The New Price is pre-filled with the unit
    ...
    def _action_update(self, order, data):
        ...
        for d in data:
            ...
            # No existing item and the user left the price at 0.00 — nothing to store.
            if not item and abs(d['new_price']) < 0.0001:
                continue
