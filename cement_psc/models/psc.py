...
class CementPSC(models.Model):
    ...
    avg_cement_conformity = fields.Selection([
        ...
        ('fail', 'Fail'),
        ('na', 'NA'),
    ], string='Conformity', compute="_compute_avg_cement_conformity")
    ...
    apparatus_counstant_lines = fields.One2many('apparatus.constant.psc.line','parent_id',string="Fineness blaine")
    ...
    detarmination_finess_lines = fields.One2many('determination.finess.psc.line','parent_id',string="Fineness blaine")
    ...
    opc_autoclave_ids = fields.One2many("mechanical.psc.autoclave.line", "parent_id", string="Test Readings")
    ...
    opc_le_method_ids = fields.One2many("mechanical.psc.lemethod.line", "parent_id", string="Test Readings")
    ...
    def _compute_avg_expantion1_conformity(self):
        for record in self:
            ...
            line = self.env['lerm.parameter.master'].sudo().search([('internal_id','=','55588996637-372f-477pp-9bcb-e9dd723001478h')])
            materials = self.env['lerm.parameter.master'].sudo().search([('internal_id','=','55588996637-372f-477pp-9bcb-e9dd723001478h')]).parameter_table
            mu_value = line.mu_value
            for material in materials:
                if material.grade.id == record.grade.id:
                    req_min = material.req_min
    ...
    consitency_of_cement_conformity = fields.Selection([
        ...
        ('fail', 'Fail'),
        ('na', 'NA'),
    ], string='Conformity', compute="_compute_consitency_of_cement_conformity")
    ...
    final_time_lines = fields.One2many('final.time.psc.line','parent_id',string="Initial Time")
    ...
    avg_specific_gravity_conformity = fields.Selection([
        ...
        ('fail', 'Fail'),
        ('na', 'NA'),
    ], string='Conformity', compute="_compute_avg_specific_gravity_conformity")
    ...
    compressive_lines = fields.One2many('compressive.psc.line','parent_id',string="Compressive")
    ...
    compressive_lines_7days = fields.One2many('compressive.line.psc.7days','parent_id',string="Compressive")
    ...
    compressive_lines_28days = fields.One2many('compressive.line.psc.28days','parent_id',string="Compressive")
    ...
    def _compute_visible(self):
        for record in self:
            ...
            for sample in record.sample_parameters:
                ...
                if sample.internal_id == '55588996637-372f-477pp-9bcb-e9dd723001478h':
                ...
                if sample.internal_id == '987psc47-5e9c-4335-9ea2-2d87624c3061':
                    record.final_setting_time_visible = True
                
                if sample.internal_id == 'psc5478-30fe-4043-b518-015f5c60d916':
                ...
                if sample.internal_id == '358789gtyg-372f-4775-9bcb-e9dd723547htui':
                    record.days_3_visible = True

                if sample.internal_id == '555888ggghhjy-372f-4775-9bcb-e9dd723547htui':
                    record.days_7_visible = True
                
                if sample.internal_id == '5777fffrrtt11-372f-4775-9bcb-e9dd723547htui':
    ...
    def open_eln_page(self):
        ...
        for result in technician_results:
            ...
            if result.parameter.internal_id == '3012478fffrr-372f-4775-9bcb-e9dd70214578r':
                ...
                result.calculated = True
                if self.fineness_blaine_avg_nabl == 'pass':
                    result.nabl_status = 'nabl'
                else:
                    result.nabl_status = 'non-nabl'
                continue
            ...
            if result.parameter.internal_id == '55588996637-372f-477pp-9bcb-e9dd723001478h':
...
class AppratusConstantLine(models.Model):
    _name = "apparatus.constant.psc.line"
    parent_id = fields.Many2one('cement.psc',string="Parent Id")
...
class DeterminationOfFinessLine(models.Model):
    _name = "determination.finess.psc.line"
    parent_id = fields.Many2one('cement.psc',string="Parent Id")
...
class CompressiveCementLine7days(models.Model):
    _name = "compressive.line.psc.7days"
    parent_id = fields.Many2one('cement.psc',string="Parent Id")
...
class CompressiveCementLine28days(models.Model):
    _name = "compressive.line.psc.28days"
    parent_id = fields.Many2one('cement.psc',string="Parent Id")
...
class CementSoundnessLeMethodLine(models.Model):
    _name = "mechanical.psc.lemethod.line"

    parent_id = fields.Many2one('cement.psc')
...
class CementSoundnessAutocalveLine(models.Model):
    _name = "mechanical.psc.autoclave.line"
