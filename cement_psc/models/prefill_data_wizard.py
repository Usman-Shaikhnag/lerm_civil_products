...
class CementPSCPrefillWizard(models.TransientModel):
    _name = 'cement.psc.prefill.data'
    ...
    def prefill_data(self):
        # Current active Cement PSC record
        current_product = self.env['cement.psc'].sudo().browse(self._context['active_id'])

        # Fetch previous Cement PSC record for selected sample
        copy_product = self.env['cement.psc'].sudo().search([
            ('eln_ref.sample_id', '=', self.sample_id.id)   # <<< FIXED
        ], limit=1)

        if not copy_product:
            raise UserError("Selected sample does not have any previous Cement PSC record.")

        # List of direct fields to copy
        normal_fields = [
            'temp_percent_setting' , 'humidity_percent_setting' , 'wt_of_empty_bottle' , 'wt_of_bottle_cement' , 'wt_of_specific_bpttle' , 'wt_of_kerosene' , 'wt_of_bottle_water' , 'wt_of_empty_bottle1' , 'wt_of_bottle_cement1' , 'wt_of_specific_bpttle1' , 'wt_of_kerosene1' , 'wt_of_bottle_water1' , 
        ]
