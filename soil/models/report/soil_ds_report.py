...
#         plt.close()
    ...
    # CBR Infra
        ...
        # Check if cbr_table exists and populate cbrx_values and cbry_values
        if general_data.cbr_field_table:
            for line in general_data.cbr_field_table:
                cbrx1.append(line.penetration1)
                cbry1.append(line.load1)
            ...
            try:
                ...
                max_y = 100  # Default value if cbry_values is empty
            try:
                min_y = round(min(cbry1), 2)
            except ValueError:
                min_y = 0
            try:
                max_x = cbrx1[cbry1.index(max_y)]
            except ValueError:
                max_x = 100
            try:
                min_x = round(min(cbrx1), 2)
            except ValueError:
                min_x = 0
            ...
            # Perform cubic spline interpolation if there are enough data points
            if len(cbrx1) > 1 and len(cbry1) > 1:
                cbrx_smooth = np.linspace(min(cbrx1), max(cbrx1), 100)
                cbrcs = CubicSpline(cbrx1, cbry1)

                # Create the line chart with a connected smooth line and markers
                plt.plot(cbrx_smooth, cbrcs(cbrx_smooth), color='red', label='Smooth Curve')
                plt.scatter(cbrx1, cbry1, marker='o', color='blue', s=30, label='Data Points')

                # Add horizontal lines with labels
                if len(cbry1) > 8:  # Ensure indices 5 and 8 exist
                    plt.axhline(y=cbry1[5], color='green', linestyle='--', label=f'Load at 2.5 mm = {cbry1[5]}')
                    plt.axhline(y=cbry1[8], color='green', linestyle='--', label=f'Load at 5 mm = {cbry1[8]}')
            ...
            buffer2 = BytesIO()
            plt.savefig(buffer2, format='png')
            cbr_graph_image_field = base64.b64encode(buffer2.getvalue()).decode('utf-8')
            plt.close()
        else:
            cbr_graph_image_field = None
            cbry1 = []  # Reset to empty list
            cbrx1 = []
        ...
        return {
            ...
            'graphCbr2': cbr_graph_image_field,
            'load21': cbry1[5] if len(cbry1) > 5 else 0,
            'load51': cbry1[8] if len(cbry1) > 8 else 0,

            'load_21': cbry1[5] if len(cbry1) > 5 else 0,
            'load5_1': cbry1[8] if len(cbry1) > 8 else 0,
        }
