...
class Soil(models.Model):
    ...
    def _compute_max_fsi_conformity(self):
        ...
        for record in self:
            ...
            line = self.env['lerm.parameter.master'].sudo().search([
                ('internal_id', '=', 'a2ae0d2c-ca64-44dd-b0ae-228aacf04998')
            ])
    ...
    def _compute_heavy_table_conformity(self):
        ...
        for record in self:
            ...
            line = self.env['lerm.parameter.master'].sudo().search([
                ('internal_id', '=', 'd5ccc1b6-20fb-4843-aa0e-2ee981be0d7c')
            ])
    ...
    light_omc_table_conformity = fields.Selection([
            ('pass', 'Pass'),
            ('fail', 'Fail'),
            ('--', '--')], string="Conformity", compute="_compute_light_omc_table_conformity", store=True)
    ...
    def _compute_light_omc_table_conformity(self):
        ...
        for record in self:
            ...
            line = self.env['lerm.parameter.master'].sudo().search([
                ('internal_id', '=', '7485d907-d8ad-4000-9376-439ef2a64c70')
            ])
            ...
            for material in materials:
                if material.grade.id == record.grade.id:
                    ...
                    if hasattr(material, 'permissable_limit') and (
                        ...
                        record.light_omc_table_conformity = '--'
                        break
    ...
    def _compute_liquid_limit_conformity(self):
        ...
        for record in self:
            ...
            line = self.env['lerm.parameter.master'].sudo().search([
                ('internal_id', '=', '8fc72243-7202-4d62-864b-8efa58b6b61f')
            ])
    ...
    def _compute_degree_of_compaction_conformity(self):
        ...
        for record in self:
            ...
            line = self.env['lerm.parameter.master'].sudo().search([
                ('internal_id', '=', 'bfc0b682-0c28-4c8b-924f-7e6988a658ee')
            ])
            ...
            for material in materials:
                if material.grade.id == record.grade.id:
                    ...
                    lower = (
                        record.avg_degree_of_compaction
                        - record.avg_degree_of_compaction * mu_value
                    )
    ...
    field_density_visible = fields.Boolean("Field Density by Sand Replacement method Visible",compute="_compute_visible")
    # job_no_dry_density = fields.Char(string="Job No")
    # material_dry_density = fields.Char(String="Material")
    # start_date_dry_density = fields.Date("Start Date")
    # end_date_dry_density = fields.Date("End Date")

    field_density_table = fields.One2many('mechanical.field.dencity.line','parent_id',string="Parameter")
    mmd_fielddencity = fields.Float(string="MMD gm/cc", store=True)
    omc_fielddencity = fields.Float(string="OMC %", store=True)
    avg_degree_of_compaction = fields.Float(string="Degree of Compaction in %",compute="_compute_avg_degree_of_compaction")

    @api.depends('field_density_table.degree_of_compaction')
    def _compute_avg_degree_of_compaction(self):
        for record in self:
            degree_of_compaction_values = record.field_density_table.mapped('degree_of_compaction')
            if degree_of_compaction_values:
                avg_degree_of_compaction = sum(degree_of_compaction_values) / len(degree_of_compaction_values)
                record.avg_degree_of_compaction = avg_degree_of_compaction
            else:
    ...
    def _compute_degree_of_compaction_conformity(self):
        ...
        for record in self:
            record.degree_of_compaction_conformity = 'fail'
            line = self.env['lerm.parameter.master'].sudo().search([('internal_id','=','2f38ad10-1ebd-4437-ba47-2633e8d9ec27')])
            materials = self.env['lerm.parameter.master'].sudo().search([('internal_id','=','2f38ad10-1ebd-4437-ba47-2633e8d9ec27')]).parameter_table
            for material in materials:
                if material.grade.id == record.grade.id:
                    # Check if permissible limit is '--' or empty
                    if hasattr(material, 'permissable_limit') and (
    ...
    moisture_content_conformity = fields.Selection([
            ('pass', 'Pass'),
            ('fail', 'Fail'),
            ('--', '--')], string="Conformity", compute="_compute_moisture_contentconformity", store=True)
    ...
    def _compute_moisture_contentconformity(self):
        ...
        for record in self:
            ...
            line = self.env['lerm.parameter.master'].sudo().search([
                ('internal_id', '=', 'a59bdedd-72cb-40e8-be97-e17fc20ff3fa')
            ])
            ...
            for material in materials:
                if material.grade.id == record.grade.id:
                    ...
                    if hasattr(material, 'permissable_limit') and (
                        ...
                        record.moisture_content_conformity = '--'
                        break
                    ...
                    lower = (
                        record.average_block
                        - record.average_block * mu_value
                    )
    ...
    cbr_field_name = fields.Char("Name",default="Determination of Field CBR")
    cbr_field_visible = fields.Boolean("Determination of Field CBR Visible",compute="_compute_visible")

    division_factor = fields.Float(
    string="Division Factor",
    digits=(8, 2),)
    
    cbr_field_table = fields.One2many('mechanical.soil.cbr.field.line','parent_id',string="CBR")
    chart_image_cbr_field = fields.Binary("Line Chart", compute="_compute_chart_image_cbr_field", store=True)


    cbr_field_ps_2mm = fields.Float("PS for 2.5mm",compute="_compute_ps_2mm_cbr_field_ps")
    cbr_field_pt_2mm = fields.Float("PT at 2.5mm",default=1343)
    cbr_field_cbr_2mm = fields.Float("CBR at 2.5mm",compute="_compute_cbr_2mm_cbr_field")

    cbr_field_ps_5mm = fields.Float("PS for 5mm",compute="_compute_ps_5mm_cbr_field")
    cbr_field_pt_5mm = fields.Float("PT at 5mm",default=2015)
    cbr_field_cbr_5mm = fields.Float("CBR at 5mm",compute="_compute_cbr_5mm_cbr_field")

    cbr_field_cbr_result = fields.Float("CBR",compute="_compute_final_cbr_field")

    @api.depends('cbr_field_table')
    def _compute_ps_2mm_cbr_field_ps(self):
        for record in self:
            if record.cbr_field_table and len(record.cbr_field_table) >= 6:
                fifth_row1 = record.cbr_field_table[5] 
                record.cbr_field_ps_2mm = fifth_row1.load1
            else:
                record.cbr_field_ps_2mm = 0


    @api.depends('cbr_field_table')
    def _compute_ps_5mm_cbr_field(self):
        for record in self:
            if record.cbr_field_table and len(record.cbr_field_table) >= 9:
                fifth_row = record.cbr_field_table[8] 
                record.cbr_field_ps_5mm = fifth_row.load1
            else:
                record.cbr_field_ps_5mm = 0

    @api.depends('cbr_field_pt_2mm','cbr_field_ps_2mm')
    def _compute_cbr_2mm_cbr_field(self):
        for record in self:
            if record.cbr_field_pt_2mm != 0:
                record.cbr_field_cbr_2mm = round((record.cbr_field_ps_2mm/record.cbr_field_pt_2mm)*100,2)
            else:
                record.cbr_field_cbr_2mm = 0

    @api.depends('cbr_field_pt_5mm','cbr_field_ps_5mm')
    def _compute_cbr_5mm_cbr_field(self):
        for record in self:
            if record.cbr_field_pt_5mm != 0:
                record.cbr_field_cbr_5mm = round((record.cbr_field_ps_5mm/record.cbr_field_pt_5mm)*100,2)
            else:
                record.cbr_field_cbr_5mm = 0

    @api.depends('cbr_field_cbr_5mm','cbr_field_cbr_2mm')
    def _compute_final_cbr_field(self):
        for record in self:
            if record.cbr_field_cbr_5mm > record.cbr_field_cbr_2mm:
                record.cbr_field_cbr_result = record.cbr_field_cbr_5mm
            else:
                record.cbr_field_cbr_result = record.cbr_field_cbr_2mm


    def generate_line_chart_cbr_field(self):
        # Prepare data for the chart
        x_values = []
        y_values = []
        for line in self.cbr_field_table:
            x_values.append(line.penetration1)
            y_values.append(line.load1)
    ...
    @api.depends('cbr_field_table')
    def _compute_chart_image_cbr_field(self):
        try:
            for record in self:
                chart_image = record.generate_line_chart_cbr_field()
                record.chart_image_cbr_field = chart_image
        except:
            pass 
    ...
    def _compute_visible(self):
        ...
        for record in self:
            ...
            record.cbr_field_visible = False
            ...
            for sample in record.sample_parameters:
                ...
                if sample.internal_id == '32145tyuj-2065-4532-814a-3a4thmvf0124':
                    record.cbr_field_visible = True
                ...
                # added
                if sample.internal_id == '2f38ad10-1ebd-4437-ba47-2633e8d9ec27':
                    record.field_density_visible = True
...
class SoilCBRLine(models.Model):
    ...
    @api.depends('proving_reading', 'parent_id.division_factor2')
    def _compute_load(self):
        for record in self:
            avg = record.proving_reading or 0.0
            factor = record.parent_id.division_factor2 or 0.0
...
class SoilInfraCBRLine(models.Model):
    ...
    @api.depends('proving_reading_avg', 'parent_id.division_factor1')
    def _compute_load(self):
        for record in self:
            avg = record.proving_reading_avg or 0.0
            factor = record.parent_id.division_factor1 or 0.0

            record.load1 = avg * factor
...
#added
class FieldDencityLine(models.Model):
    _name = "mechanical.field.dencity.line"
    parent_id = fields.Many2one('mechanical.soil',string="Parent Id")
    ...
    def create(self, vals):
        ...
        return super(FieldDencityLine, self).create(vals)
...
class SoilFieldCBRLine(models.Model):
    _name = "mechanical.soil.cbr.field.line"
    parent_id = fields.Many2one('mechanical.soil',string="Parent Id")
    ...
    proving_reading_avg = fields.Float(string="Proving Ring Reading Avg.",compute="_compute_avg_reading_cbr",digits=(12,2))

    load1 = fields.Float(string="Load in Kg", compute="_compute_load_cbr")


    # @api.depends('proving_reading1', 'proving_reading2', 'proving_reading3')
    # def _compute_avg_reading_cbr(self):
    #     for rec in self:
    #         readings = [rec.proving_reading1, rec.proving_reading2, rec.proving_reading3]
    #         if any(readings):
    #             avg = sum(readings) / 3
    ...
    def _compute_avg_reading_cbr(self):
        for rec in self:
            readings = [
                rec.proving_reading1 or 0.0,
                rec.proving_reading2 or 0.0,
                rec.proving_reading3 or 0.0
            ]

            if any(readings):
                rec.proving_reading_avg = sum(readings) / 3   
            else:
                rec.proving_reading_avg = 0.0


    # @api.depends('proving_reading_avg','parent_id.division_factor')
    # def _compute_load_cbr(self):
    #     for record in self:
    #         record.load1 = record.proving_reading_avg * parent_id.division_factor
