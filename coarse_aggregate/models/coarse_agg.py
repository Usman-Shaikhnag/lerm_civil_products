...
class CoarseAggregateMechanical(models.Model):
   ...
   # Bulk Density
    loose_bulk_density_name1 = fields.Char("Name",default="Loose Bulk Density")
    ...
    avg_bulk_density_conformity = fields.Selection([
            ('pass', 'Pass'),
            ('fail', 'Fail'),
            ('na', 'NA'),
            ], string="Loose Bulk Conformity", compute="_compute_avg_bulk_density_conformity", store=True)
    

    avg_bulk_density_nabl = fields.Selection([
        ('pass', 'NABL'),
        ('fail', 'Non-NABL')], string="Loose Bulk NABL", compute="_compute_avg_bulk_density_nabl", store=True)
    ...
    def _compute_avg_bulk_density_conformity(self):
        
        for record in self:
            if not record.eln_ref or not record.eln_ref.conformity:
                record.avg_bulk_density_conformity = 'na'
                continue
            record.avg_bulk_density_conformity = 'fail'
            ...
            for material in materials:
                # if material.grade.id == record.grade.id:
                    ...
                    if lower >= req_min and upper <= req_max:
                        record.avg_bulk_density_conformity = 'pass'
                        break
                    else:
                        record.avg_bulk_density_conformity = 'fail'
    ...
    avg_compacted_conformity = fields.Selection([
            ...
            ('fail', 'Fail'),
            ('na', 'NA'),
            ], string="Compacted Bulk Conformity", compute="_compute_avg_compacted_conformity", store=True)
    ...
    def _compute_angularity_number_conformity(self):
        ...
        for record in self:
            if not record.eln_ref or not record.eln_ref.conformity:
                record.angularity_number_conformity = 'na'
                continue
