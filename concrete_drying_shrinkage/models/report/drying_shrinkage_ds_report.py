...
class DryingShrinkageDatasheet(models.AbstractModel):
    _name = 'report.concrete_drying_shrinkage.drying_shrinkage_datasheet'
...
class DryingShrinkageReport(models.AbstractModel):
    _name = 'report.concrete_drying_shrinkage.drying_shrinkage_repprt'
    ...
    def _get_report_values(self, docids, data):
        ...
        drying_data = self.env[model].sudo().search([("id","=",eln.model_id)])
        return {
            'eln': eln,
            'drying': drying_data,
            'qrcode': qr_code,
