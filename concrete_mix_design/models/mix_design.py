...
class ConcreteDesign(models.Model):
    ...
    _inherit = "lerm.eln"
    _description = 'mechanical.concrete.design'
    _rec_name = "name"
    ...
    child_lines_concrete_design = fields.One2many('mechanical.concrete.design.line','parent_id',string=" CONCRETE MIX DESIGN",default=lambda self: self._default_concrete_child_lines())
...
# SPECIFIC GRAVITY & WATER ABSORPTION OF COARSE & FINE AGGREGATE (SSD)
    ...
    child_lines_concrete_design_material = fields.One2many('mechanical.concrete.design.material.line','parent_id',string="Material Table")
    ...
    # Sieve size 20 MM
    sieve_analysis_child_lines_20mm = fields.One2many('mechanical.concrete.design.sieve.20mm.line','parent_id',string="Parameter",default=lambda self: self._default_sieve_analysis_child_lines20mm())
        ...
        for record in self:
            for line in record.sieve_analysis_child_lines_20mm:
                ...
                if previous_line == 0:
                    ...
                    previous_line_record = self.env['mechanical.concrete.design.sieve.20mm.line'].sudo().search([("serial_no", "=", previous_line),("parent_id","=",self.id)]).cumulative_retained
    ...
    sieve_analysis_child_lines = fields.One2many('mechanical.concrete.design.sieve.analysis.line','parent_id',string="Parameter",default=lambda self: self._default_sieve_analysis_child_lines10mm())
        ...
        for record in self:
            for line in record.sieve_analysis_child_lines:
                ...
                if previous_line == 0:
                    ...
                    previous_line_record = self.env['mechanical.concrete.design.sieve.analysis.line'].sudo().search([("serial_no", "=", previous_line),("parent_id","=",self.id)]).cumulative_retained
                    line.write({'cumulative_retained': previous_line_record + line.percent_retained})
    ...
    child_lines = fields.One2many('mechanical.concrete.design.crush.sand.line','parent_id',string="Sieve Analysis",default=lambda self: self._default_sieve_analysis_child_lines())
        ...
        for record in self:
            for line in record.child_lines:
                ...
                if previous_line == 0:
                    ...
                    previous_line_record = self.env['mechanical.concrete.design.crush.sand.line'].sudo().search([("serial_no", "=", previous_line),("parent_id","=",self.id)]).cumulative_retained
                    line.write({'cumulative_retained': previous_line_record + line.percent_retained})
                    line.write({'passing_percent': 100-(previous_line_record + line.percent_retained)})
                    print("Previous Cumulative",previous_line_record)
    ...
    combination10and20mm_child_lines = fields.One2many('mechanical.combination.10and20.line','parent_id',string="Sieve Analysis Combination",default=lambda self: self._default_combination10and20mm_child_lines())
    ...
    combine_granding_child_lines = fields.One2many('mechanical.combined.granding.line','parent_id',string="Sieve Analysis Combination",default=lambda self: self._default_combine_granding_child_lines())
    ...
    def get_all_fields(self):
        record = self.env['mechanical.concrete.design'].browse(self.ids[0])
        field_values = {}
    ...
    notes_id = fields.One2many('mechanical.concrete.design.notes', 'parent_id', string="Notes", default=lambda self: self._default_notes_lines())
...
class SieveAnalysisLine(models.Model):
    _name = "mechanical.concrete.design.sieve.analysis.line"
...
class SieveAnalysisLine20mm(models.Model):
    _name = "mechanical.concrete.design.sieve.20mm.line"
    parent_id = fields.Many2one('mechanical.concrete.design', string="Parent Id")
...
class CrushSandLine(models.Model):
    _name = "mechanical.concrete.design.crush.sand.line"
    parent_id = fields.Many2one('mechanical.concrete.design', string="Parent Id")
...
class Concrete_Design_FLine(models.Model):
    _name = "mechanical.concrete.design.line"
    parent_id = fields.Many2one('mechanical.concrete.design', string="Parent Id")
...
class ConcreteDesignMaterialLine(models.Model):
    _name = "mechanical.concrete.design.material.line"
    parent_id = fields.Many2one('mechanical.concrete.design', string="Parent Id")
...
class Combination10And20Line(models.Model):
    _name = "mechanical.combination.10and20.line"
    parent_id = fields.Many2one('mechanical.concrete.design', string="Parent Id")
...
class CombinedGrandingLine(models.Model):
    _name = "mechanical.combined.granding.line"
    parent_id = fields.Many2one('mechanical.concrete.design', string="Parent Id")
...
class ConcreteDesignNotes(models.Model):
    _name = "mechanical.concrete.design.notes"
