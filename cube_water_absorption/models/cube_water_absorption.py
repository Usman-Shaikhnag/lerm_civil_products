...
class MechanicalCubeWater(models.Model):
    _name = "cube.water.absorption"
    _inherit = "lerm.eln"
    _rec_name = "name"
    ...
    child_lines = fields.One2many('cube.water.absorption.line','parent_id',string="Parameter")
    average_concrete_cube_water = fields.Float(string="Average Water Absorption %", compute="_compute_average_concrete_cube_water1",digits=(12,2))
    correction_factor = fields.Float(string="Correction Factor As per BS 1881 Part 122")
    water_correction_factor = fields.Float(string="Water Absorption after correction Factor",compute="_compute_water_correction_factor1")
    ...
    notes_id = fields.One2many('cube.water.absorption.notes', 'parent_id', string="Notes",ondelete='cascade')
    
    @api.model
    def default_get(self, fields):
        res = super(MechanicalCubeWater, self).default_get(fields)
    ...
    def _compute_nabl(self):
        ...
        for record in self:
            record.nabl = 'fail'
            line = self.env['lerm.parameter.master'].sudo().search([('internal_id','=','90876tgbv-b01c-47c1-a37c-332e37a8765g')])
            materials = self.env['lerm.parameter.master'].sudo().search([('internal_id','=','90876tgbv-b01c-47c1-a37c-332e37a8765g')]).parameter_table
    ...
    def _compute_average_concrete_cube_water1(self):
        for record in self:
    ...
    def _compute_water_correction_factor1(self):
        for record in self:
    ...
    def open_eln_page(self):
        ...
        for result in technician_results:
            ...
            if result.parameter.internal_id == '90876tgbv-b01c-47c1-a37c-332e37a8765g':
                result.result_char = round(self.average_concrete_cube_water,2)
    ...
    def create(self, vals):
        # import wdb;wdb.set_trace()
        record = super(MechanicalCubeWater, self).create(vals)
    ...
    def get_all_fields(self):
        record = self.env['cube.water.absorption'].browse(self.ids[0])
        field_values = {}
...
class MechanicalCubeWaterLine(models.Model):
    _name = "cube.water.absorption.line"
    parent_id = fields.Many2one('cube.water.absorption',string="Parent Id")

    sr_no = fields.Integer(string="Sr.No.",readonly=True, copy=False, default=1)
    sample_id = fields.Char(string="Sample ID")
    dia_specimen = fields.Float(string="Diameter of Specimen (mm)" ,digits=(12,2))
    length_specimen = fields.Float(string="Length of Specimen (mm)" ,digits=(12,2))
    ...
    water_absorption_percent = fields.Float(string="Water Absorption %",compute="_compute_water_absorption_percent1" ,digits=(12,2))
    ...
    def _onchange_sample_id(self):
        for record in self:
            if record.sample_id and not record.parent_id.eln_ref.sample_id.client_sample_id:
                record.parent_id.eln_ref.sample_id.client_sample_id = record.sample_id
    ...
    def _compute_water_absorption_percent1(self):
        for record in self:
    ...
    def create(self, vals):
        ...
        return super(MechanicalCubeWaterLine, self).create(vals)
...
class ConcreteCubeWaterNotes(models.Model):
    _name = "cube.water.absorption.notes"
