...
class BitumenConcretePrefillWizard(models.TransientModel):
    _name = 'bitumen.concrete.aggregate.prefill.data'
    ...
    def prefill_data(self):
        # Current active Bitumen Concrete record
        current_product = self.env['mechanical.bitumen.concrete'].sudo().browse(self._context['active_id'])

        # Fetch previous Bitumen Concrete record for selected sample
        ...
        normal_fields = [
            'location' , 'location_heding' , 'wt_of_samplew1' , 'wt_of_intial' ,  'wt_of_aggregate' , 'wt_of_extraction' , 'location' , 'wt_of_sample' ,  
        ]
