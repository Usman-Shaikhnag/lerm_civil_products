...
class BitumenConcrete(models.Model):
    ...
    name_bitumen1 = fields.Char("Name",default="Bitumen Concrete")
...
#    Specific of Gravity of Bitumen

    specific_gravity_name1 = fields.Char("Name",default="Specific  Gravity of Bitumen")
    specific_gravity_visible = fields.Boolean("Specific of Gravity of Bitumen",compute="_compute_visible")

    specific_gravity_lines = fields.One2many('mechanical.bitumen.concrete.sg.line','parent_id',string="Parameter")

    avg_sp_bitumen = fields.Float(string="Average Specific Gravity",compute="_compute_avg_sp_bitumen", store=True,digits=(12,3))
        ...
        for record in self:
            ...
            for material in materials:
                if material.grade.id == record.grade.id:
                    ...
                    lower = record.avg_sp_bitumen - record.avg_sp_bitumen*mu_value
                    upper = record.avg_sp_bitumen + record.avg_sp_bitumen*mu_value
                    if lower >= req_min and upper <= req_max:
                        record.avg_sp_bitumen_conformity = 'pass'
                        break
                    else:
                        record.avg_sp_bitumen_conformity = 'fail'

    avg_sp_bitumen_nabl = fields.Selection([
        ('pass', 'NABL'),
        ('fail', 'Non-NABL')], string="NABL", compute="_compute_avg_sp_bitumen_nabl", store=True)

    @api.depends('avg_sp_bitumen','eln_ref','grade')
    def _compute_avg_sp_bitumen_nabl(self):
        
        for record in self:
            record.avg_sp_bitumen_nabl = 'fail'
            line = self.env['lerm.parameter.master'].sudo().search([('internal_id','=','5487698-4587-4086-b132-62b50e63f1247ui')])
            materials = self.env['lerm.parameter.master'].sudo().search([('internal_id','=','5487698-4587-4086-b132-62b50e63f1247ui')]).parameter_table
            for material in materials:
                if material.grade.id == record.grade.id:
                    ...
                    upper = record.avg_sp_bitumen + record.avg_sp_bitumen*mu_value
                    if lower >= lab_min and upper <= lab_max:
                        record.avg_sp_bitumen_nabl = 'pass'
                        break
                    else:
                        record.avg_sp_bitumen_nabl = 'fail'

    @api.depends('specific_gravity_lines.sp_bitumen')
    def _compute_avg_sp_bitumen(self):
        for rec in self:
            bitumen_values = [line.sp_bitumen for line in rec.specific_gravity_lines if line.sp_bitumen]
            if bitumen_values:
                rec.avg_sp_bitumen = sum(bitumen_values) / len(bitumen_values)
            else:
                rec.avg_sp_bitumen = 0.0


  

    # wt_of_sp_stoper1 = fields.Float(string=" weight the specific gravity bottle alongwith the stopper (Weight 'A').")
    # wt_of_sp_bottle1 = fields.Float(string=" weight specific gravity bottle + distilled water")
    # wt_of_sp_material1 = fields.Float(string=" Weight  the specific gravity bottle + half-filled with the material")
    # wt_of_sp_water1 = fields.Float(string="Weight the specific gravity bottle + half-filled with the material + half with distilled water")
    # sp_bitumen1 = fields.Float(string="Specific gravity of bitumen =(C-A)/(B-A)-(D-C)",compute="_compute_sp_bitumen1",store=True,digits=(12,3))

    # @api.depends('wt_of_sp_stoper1', 'wt_of_sp_bottle1', 'wt_of_sp_material1', 'wt_of_sp_water1')
    # def _compute_sp_bitumen1(self):
    #     for rec in self:
    #         A = rec.wt_of_sp_stoper1
    #         B = rec.wt_of_sp_bottle1
    #         C = rec.wt_of_sp_material1
    #         D = rec.wt_of_sp_water1

    #         denominator = (B - A) - (D - C)
    #         if denominator != 0:
    #             rec.sp_bitumen1 = (C - A) / denominator
    #         else:
    #             rec.sp_bitumen1 = 0.0

    
    
    # @api.depends('sp_bitumen')
    # def _compute_avg_sp_bitumen(self):
    #     for rec in self:
    #         values = [rec.sp_bitumen, rec.sp_bitumen1]
    #         valid_values = [v for v in values if v not in (False, 0.0)]
    #         if valid_values:
    #             rec.avg_sp_bitumen = sum(valid_values) / len(valid_values)
    #         else:
    #             rec.avg_sp_bitumen = 0.0


    flash_and_fire_name = fields.Char("Name",default="Flash and fire point  of Bitumen")
    flash_and_fire_visible = fields.Boolean("Flash and fire point  of Bitumen",compute="_compute_visible")

    flash_and_fire_lines = fields.One2many('mechanical.bitumen.concrete.ff.line','parent_id',string="Parameter")
    ...
    def _compute_mean_flash(self):
        for rec in self:
            flash_points = [line.flash_point for line in rec.flash_and_fire_lines if line.flash_point]
            if flash_points:
                rec.mean_flash = sum(flash_points) / len(flash_points)
            else:
                rec.mean_flash = 0.0

    @api.depends('flash_and_fire_lines.fire_point')
    def _compute_mean_fire(self):
        for rec in self:
            fire_points = [line.fire_point for line in rec.flash_and_fire_lines if line.fire_point]
            if fire_points:
                rec.mean_fire = sum(fire_points) / len(fire_points)
            else:
                rec.mean_fire = 0.0


    softining_point_name = fields.Char("Name",default="Softining point of Bitumen")
    softining_point_visible = fields.Boolean("Softining point of Bitumen",compute="_compute_visible")

    softining_point_lines = fields.One2many('mechanical.bitumen.concrete.softining.line','parent_id',string="Parameter")

    avg_temp = fields.Float(string="Avg temp",compute="_computeavg_temp",digits=(12,2),store=True)
    ...
    def _computeavg_temp(self):
        for rec in self:
            softining_points = [line.softining_point for line in rec.softining_point_lines if line.softining_point]
            if softining_points:
                rec.avg_temp = sum(softining_points) / len(softining_points)
            else:
                rec.avg_temp = 0.0



    penetration_name = fields.Char("Name",default="Penetration of Bitumen")
    penetration_visible = fields.Boolean("Penetration of Bitumen",compute="_compute_visible")

    penetration_lines = fields.One2many('mechanical.bitumen.concrete.penetration.line','parent_id',string="Parameter")
    ...
    def _compute_avg_penetration(self):
        for rec in self:
            penetrations = rec.penetration_lines.filtered(lambda l: l.penetration is not None)
            total = sum(line.penetration for line in penetrations)
            count = len(penetrations)
            rec.avg_penetration = total / count if count > 0 else 0.0
        ...
        for record in self:
            ...
            for line in record.sieve_analysis_child_lines:
                ...
                if previous_line == 0:
                    ...
                    previous_line_record = self.env['mechanical.bitumen.concrete..sieve.line'].sudo().search([
    ...
    def _compute_visible(self):
        ...
        for record in self:
            ...
            record.flash_and_fire_visible = False
            record.softining_point_visible = False
            record.penetration_visible = False
            ...
            for sample in record.sample_parameters:
                ...
                if sample.internal_id == "32578999-7188-4086-b132-62b50e63f1247ui":
                    record.bitumen_content_visible = True
                
                if sample.internal_id == "5487698-4587-4086-b132-62b50e63f1247ui":
                    record.specific_gravity_visible = True

                if sample.internal_id == "6587412-4587-4086-b132-62b50e63f124778":
                    record.flash_and_fire_visible = True

                if sample.internal_id == "3587914-4587-4086-b132-62b50e63f124772":
                    record.softining_point_visible = True

                if sample.internal_id == "68574nht-4587-4086-b132-62b50e63f12474h":
                    record.penetration_visible = True
...
class SpecificGravitytLine(models.Model):
    _name = "mechanical.bitumen.concrete.sg.line"
    parent_id = fields.Many2one('mechanical.bitumen.concrete',string="Parent Id")

    serial_no = fields.Integer(string="Sample", readonly=True, copy=False, default=1)
    wt_of_sp_stoper = fields.Float(string=" weight the specific gravity bottle alongwith the stopper (Weight 'A').")
    wt_of_sp_bottle = fields.Float(string=" weight specific gravity bottle + distilled water")
    wt_of_sp_material = fields.Float(string=" Weight  the specific gravity bottle + half-filled with the material")
    wt_of_sp_water = fields.Float(string="Weight the specific gravity bottle + half-filled with the material + half with distilled water")
    sp_bitumen = fields.Float(string="Specific gravity of bitumen =(C-A)/(B-A)-(D-C)",compute="_compute_sp_bitumen",store=True,digits=(12,3))

    @api.depends('wt_of_sp_stoper', 'wt_of_sp_bottle', 'wt_of_sp_material', 'wt_of_sp_water')
    def _compute_sp_bitumen(self):
        for rec in self:
            A = rec.wt_of_sp_stoper
            B = rec.wt_of_sp_bottle
            C = rec.wt_of_sp_material
            D = rec.wt_of_sp_water

            denominator = (B - A) - (D - C)
            if denominator != 0:
                rec.sp_bitumen = (C - A) / denominator
            else:
                rec.sp_bitumen = 0.0
    ...
    def create(self, vals):
        ...
        return super(SpecificGravitytLine, self).create(vals)
...
class FlashFireLine(models.Model):
    _name = "mechanical.bitumen.concrete.ff.line"
    parent_id = fields.Many2one('mechanical.bitumen.concrete',string="Parent Id")

    serial_no = fields.Integer(string="Trails", readonly=True, copy=False, default=1)
    flash_point = fields.Float(string="Flash point")
    fire_point = fields.Float(string="Fire Point")
    ...
    def create(self, vals):
        ...
        return super(FlashFireLine, self).create(vals)
...
class SoftinigPointLine(models.Model):
    _name = "mechanical.bitumen.concrete.softining.line"
    parent_id = fields.Many2one('mechanical.bitumen.concrete',string="Parent Id")

    serial_no = fields.Integer(string="Ball", readonly=True, copy=False, default=1)
    softining_point = fields.Float(string="Softning Point Temp")
    ...
    def create(self, vals):
        ...
        return super(SoftinigPointLine, self).create(vals)
...
class PenetrationLine(models.Model):
    _name = "mechanical.bitumen.concrete.penetration.line"
    parent_id = fields.Many2one('mechanical.bitumen.concrete',string="Parent Id")

    serial_no = fields.Integer(string="sample", readonly=True, copy=False, default=1)
    penetration_c = fields.Float(string="Temp of penetration in 'c")
    penetration_sec = fields.Float(string="Temp of penetration in sec ")
    penetration_initial = fields.Float(string="Penetration Initial")
    penetration_final = fields.Float(string="Penetration Final")
    penetration = fields.Float(string="Penetration",compute="_compute_penetration",store=True)

    @api.depends('penetration_final', 'penetration_initial')
    def _compute_penetration(self):
        for rec in self:
            rec.penetration = (rec.penetration_final or 0) - (rec.penetration_initial or 0)
    ...
    def create(self, vals):
        ...
        return super(PenetrationLine, self).create(vals)
