...
class FlexuralStrengthConcreteBeam(models.Model):
    ...
    flexural_strength_name = fields.Char("Name", default="Drying Shrinkage")
    flexural_strength_visible = fields.Boolean("Drying Shrinkage", compute="_compute_visible")
    ...
    #     for record in self:
    #         record.flexural_strength = False
       
           

    #         for sample in record.sample_parameters:
    #             print("Internal Ids",sample.internal_id)
    #             if sample.internal_id == "a3c54810-53a6-4113-9c05-c996fb0c34c9":
    #                 record.flexural_strength = True


    notes_id = fields.One2many('flexural.strength.notes', 'parent_id', string="Notes")
    ...
    #         line = self.env['lerm.parameter.master'].search([('internal_id','=','a3c54810-53a6-4113-9C05-c996fb0c34c9')])
    #         materials = self.env['lerm.parameter.master'].search([('internal_id','=','a3c54810-53a6-4113-9C05-c996fb0c34c9')]).parameter_table
    ...
    flexural_strength_conformity = fields.Selection([
            ('pass', 'Pass'),
            ('fail', 'Fail'),
        ('--', '--')
        ], string="Conformity", compute="_compute_flexural_strength_conformity", store=True)

    @api.depends('average_flexural_strength','eln_ref','grade')
    def _compute_flexural_strength_conformity(self):
        
        for record in self:
            record.flexural_strength_conformity = 'fail'
            line = self.env['lerm.parameter.master'].sudo().search([('internal_id','=','a3c54810-53a6-4113-9C05-c996fb0c34c9')])
            materials = self.env['lerm.parameter.master'].sudo().search([('internal_id','=','a3c54810-53a6-4113-9C05-c996fb0c34c9')]).parameter_table
            for material in materials:
                if material.grade.id == record.grade.id:
                    ...
                    if hasattr(material, 'permissable_limit') and (material.permissable_limit == '--' or not material.permissable_limit):
                        record.flexural_strength_conformity = '--'
                        break
                    ...
                    upper = record.average_flexural_strength + record.average_flexural_strength*mu_value
                    if lower >= req_min and upper <= req_max:
                        record.flexural_strength_conformity = 'pass'
                        break
                    else:
                        record.flexural_strength_conformity = 'fail'

    flexural_strength_nabl = fields.Selection([
        ('pass', 'NABL'),
        ('fail', 'Non-NABL')], string="NABL", compute="_compute_flexural_strength_nabl", store=True)

    @api.depends('average_flexural_strength','eln_ref','grade')
    def _compute_flexural_strength_nabl(self):
        
        for record in self:
            record.flexural_strength_nabl = 'fail'
            line = self.env['lerm.parameter.master'].sudo().search([('internal_id','=','a3c54810-53a6-4113-9C05-c996fb0c34c9')])
            materials = self.env['lerm.parameter.master'].sudo().search([('internal_id','=','a3c54810-53a6-4113-9C05-c996fb0c34c9')]).parameter_table
            # for material in materials:
            #     if material.grade.id == record.grade.id:
            ...
            if lower >= lab_min and upper <= lab_max:
                record.flexural_strength_nabl = 'pass'
                break
            else:
                record.flexural_strength_nabl = 'fail'
    ...
    def open_eln_page(self):
        ...
        for result in technician_results:
            ...
            if result.parameter.internal_id == 'a3c54810-53a6-4113-9C05-c996fb0c34c9':
                result.result_char = round(self.average_flexural_strength,2)
                result.calculated = True
                if self.flexural_strength_nabl == 'pass':
